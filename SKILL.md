...
**Key insight**: Memories are matched by *search terms*, not by project names. A memory for "looker-automation script running in terminal" matches the Lightspeed DMS project not because it says "Lightspeed" but because "looker-automation" is a known search term for that project. The skill's intelligence lives in building and refining these search term lists.
...
- **Good terms**: Specific tool names ("looker-automation"), file names ("launch-dashboard.js"), unique client identifiers ("lightspeed"), technical terms tied to the project ("universal linking", "bigquery dataset")
...
- **High**: Memory text contains a unique, specific term (e.g., "looker-automation" → Lightspeed DMS). Only one project could match.
